...
impl SharedMemory {
    pub fn new(title: impl Into<String>, content: impl Into<String>, tags: Vec<String>, emotional_weight: f32) -> Self {
        Self {
            ts: Utc::now(),
            title: title.into(),
    ...
    pub fn relevance_score(&self, input: &str) -> f32 {
        println!("[MEMORY_SCORE] Calculating relevance for memory: \"{}\"", self.title);
        let input_lc = input.to_ascii_lowercase();
        let mut score = self.emotional_weight * 0.40;
        println!("[MEMORY_SCORE] Base score from emotional_weight ({}): {}", self.emotional_weight, score);
        ...
        if !self.content.is_empty() {
            ...
            let matching_words: Vec<String> = c.split_whitespace()
                .filter(|w| w.len() > 4 && input_lc.contains(w))
                .map(|w| w.to_string())
                .collect();
            
            if !matching_words.is_empty() {
                println!("[MEMORY_SCORE] Content word matches: {}. Adding 0.15", matching_words.join(", "));
                score += 0.15;
            }
        }
        
        let final_score = score.clamp(0.0, 1.0);
        println!("[MEMORY_SCORE] Final relevance score for \"{}\": {}", self.title, final_score);
        final_score
    }
}
